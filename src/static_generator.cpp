...
std::string generate_mime_extension(std::string file_request) {
...
void static_one_file_generator(Request req, std::string file_request) {
  std::ifstream file(file_request.c_str());
  std::string file_string((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
  std::string mime = generate_mime_extension(file_string);

  req.createRespond(200, file_string, mime);
  req.respond();
}
...
void static_generator(Request req, std::string base) {
  ...
  if ((open((base + file_request).c_str(), O_RDONLY)) == -1) {
    ...
    std::string file_string((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
    std::string file_mime = generate_mime_extension(file_request);
